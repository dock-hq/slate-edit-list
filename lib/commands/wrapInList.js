...
const getHighestSelectedElements = (options: Options) => (
  ...
  const startPath = Range.start(selection).path;
  const endPath = Range.end(selection).path;
  ...
  const startIndex = Path.relative(startPath, ancestorPath)[0];
  const endIndex = Path.relative(endPath, ancestorPath)[0];
  ...
  const hasContainerBlocks = siblings.some(([siblingNode, siblingPath]) => {
    ...
    const startPath = Range.start(selection).path;
    const endPath = Range.end(selection).path;
    const isStartDescendant = Path.isDescendant(startPath, siblingPath);
    const isEndDescendant = Path.isDescendant(endPath, siblingPath);
    ...
    const startBlockLevels = countBlockLevels(startPath);
    const endBlockLevels = countBlockLevels(endPath);
